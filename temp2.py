...
def getVetor():
    ...
    conexao = pymysql.connect(db='deriv', user='root', passwd='root')
    cursor = conexao.cursor()
    cursor.execute("select valor from preco where id%5 = 0 ORDER BY id desc LIMIT "+str(timestamp)+";")
    tmp = cursor.fetchall()
    for k in tmp:
        v.append(float(str(k).replace("(","").replace(")","").replace(",","")))
    s = v[::-1]
...
def getDados():
    ...
    conexao = pymysql.connect(db='deriv', user='root', passwd='root')
    cursor = conexao.cursor()
    cursor.execute("select valor from preco WHERE id%5 = 0 ORDER BY id desc LIMIT "+str(100)+";")
    tmp = cursor.fetchall()
    for k in tmp:
        v.append(float(str(k).replace("(","").replace(")","").replace(",","")))
    v = v[::-1]
...
def op():
    global driver,s,v,timestamp,tempoFrente, ca, ve, win, loss
    ...
    while(True):
        ...
        a = model.predict(a)
        a = a[0][0]
        print([s[timestamp-1],a, win, loss])
        valor  = float(driver.find_elements_by_class_name("cq-current-price")[0].text)
            
        if(valor > a > s[timestamp-1]):
            print(1)
            co.click()
        if(valor < a < s[timestamp-1]):
            print(2)
            ve.click()
        if(s[timestamp-1] > a > valor):
            print(3)
            co.click()
        if(s[timestamp-1] > a > valor):
            print(4)
